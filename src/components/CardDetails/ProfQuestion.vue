...
export default {
    ...
    methods: {
        ...
        saveToLocalStorage() {
            const savedState = JSON.parse(localStorage.getItem('questionStates')) || {};
            savedState[this.quest] = {
                isLiked: this.localIsLiked,
                likeQuantity: this.localLikeQuantity,
                isPined: this.localIsPined,
            };
            localStorage.setItem('questionStates', JSON.stringify(savedState));
        },
        loadFromLocalStorage() {
            const savedState = JSON.parse(localStorage.getItem('questionStates')) || {};
            const state = savedState[this.quest];
            if (state) {
                this.localIsLiked = state.isLiked;
                this.localIsPined = state.isPined;
                this.localLikeQuantity = state.likeQuantity;
            } else {
                this.localIsLiked = this.isLiked;
                this.localIsPined = this.isPined;
            }
        },
    },
    watch: {
...
<template>
  <div :class="['flex flex-col md:w-12/12 max-[767px]:w-full bg-[#FCFCFC] border border-[#000000] rounded-md mt-[10px] drop-shadow-lg transition-all duration-300', 
    ...
    <div v-if="isOpen" class="w-full">
        ...
        <div class="flex justify-center items-center py-2 w-[80%] ml-[10%]">
            <p class="text-[15px] text-[#514E4E] max-[767px]:text-[11px] max-[767px]:mt-[5px] flex items-center">{{text}}</p>
        </div>
    </div>
  </div>
</template>
